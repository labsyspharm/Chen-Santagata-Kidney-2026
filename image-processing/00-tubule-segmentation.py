import json
import pathlib
import re

import cv2
import dask.array as da
import joblib
import matplotlib.patches as mpatches
import matplotlib.pyplot as plt
import numpy as np
import palom
import pandas as pd
import scipy.spatial.distance as sdistance
import shapely
import shapely.validation
import skimage.filters
import skimage.morphology
import skimage.segmentation
import tifffile


def parse_roi_points(all_points):
    return np.array(re.findall(r"-?\d+\.?\d+", all_points), dtype=float).reshape(-1, 2)


def ellipse_points_to_patch(
    vertex_1, vertex_2, co_vertex_1, co_vertex_2, patch_kwargs={}
):
    """
    Parameters
    ----------
    vertex_1, vertex_2, co_vertex_1, co_vertex_2: array like, in the form of (x-coordinate, y-coordinate)

    """
    v_and_co_v = np.array([vertex_1, vertex_2, co_vertex_1, co_vertex_2])
    centers = v_and_co_v.mean(axis=0)

    d = sdistance.cdist(v_and_co_v, v_and_co_v, metric="euclidean")
    width = d[0, 1]
    height = d[2, 3]

    vector_2 = v_and_co_v[1] - v_and_co_v[0]
    vector_2 /= np.linalg.norm(vector_2)

    angle = np.degrees(np.arccos([1, 0] @ vector_2))

    ellipse_patch = mpatches.Ellipse(
        centers, width=width, height=height, angle=angle, **patch_kwargs
    )
    return ellipse_patch


def add_mpatch(roi, patch_kwargs=None, scale=1):
    if patch_kwargs is None:
        patch_kwargs = {}
    roi = roi.copy()
    points = parse_roi_points(roi["all_points"])
    points *= scale
    roi.loc["parsed_points"] = points

    roi_type = roi["type"]
    if roi_type in ["Point", "Line"]:
        roi_mpatch = mpatches.Polygon(points, closed=False, **patch_kwargs)
    elif roi_type in ["Rectangle", "Polygon", "Polyline"]:
        roi_mpatch = mpatches.Polygon(points, closed=True, **patch_kwargs)
    elif roi_type == "Ellipse":
        roi_mpatch = ellipse_points_to_patch(*points, patch_kwargs=patch_kwargs)
    else:
        raise ValueError

    roi.loc["mpatch"] = roi_mpatch
    return roi


def glomerulus_roi_path_to_mask(roi_path, mask_shape=None, buffer_size=0):
    _roi = pd.read_csv(roi_path)
    is_polygon = _roi["type"].fillna("").astype("str").apply(lambda x: x == "Polyline")
    assert is_polygon.sum() <= 255
    roi = _roi.loc[is_polygon].apply(add_mpatch, axis=1)

    polygons = [
        to_valid_polygon(patch_to_shapely_polygon(pp).buffer(buffer_size))
        for pp in roi["mpatch"].dropna()
    ]
    mask = shapely_polygons_to_mask(polygons, mask_shape, 1)

    indexer = np.zeros(mask.max() + 1, dtype="uint8")
    for ii, tt in enumerate(roi["Text"].fillna("GL")):
        tt = tt.upper()
        assert tt in ["GL", "GS"]
        if tt == "GL":
            indexer[ii + 1] = 1
        else:
            indexer[ii + 1] = 2

    _mask = indexer[mask]
    # return (glomeruli-mask, sclerosed-glomeruli-mask)
    return (np.where(_mask == 1, mask, 0), np.where(_mask == 2, mask, 0))


def roi_path_to_mask(roi_path, mask_shape=None, fill_value_min=None, buffer_size=0):
    _roi = pd.read_csv(roi_path)
    is_polygon = _roi["type"].fillna("").astype("str").apply(lambda x: x == "Polyline")
    assert len(_roi) <= 255
    roi = _roi.loc[is_polygon].apply(add_mpatch, axis=1)

    polygons = [
        to_valid_polygon(patch_to_shapely_polygon(pp).buffer(buffer_size))
        for pp in roi["mpatch"].dropna()
    ]
    return shapely_polygons_to_mask(polygons, mask_shape, fill_value_min)


def shapely_polygons_to_mask(polygons, mask_shape=None, fill_value_min=None):
    coords = [np.array(pp.exterior.coords) for pp in polygons]

    if mask_shape is None:
        mask_shape = np.ceil(np.vstack(coords).max(axis=0)).astype("int")[::-1]
    h, w = mask_shape

    if fill_value_min is None:
        fill_value_min = 1
    fill_value = int(fill_value_min)

    mask = np.zeros((int(h), int(w)), np.uint8)
    for pp in coords:
        cv2.fillPoly(mask, [pp.round().astype("int")], fill_value)
        fill_value += 1
    return mask


def to_valid_polygon(polygon: shapely.Polygon):
    polygon = shapely.validation.make_valid(polygon)

    if isinstance(polygon, shapely.Polygon):
        return polygon

    elif isinstance(polygon, (shapely.MultiPolygon, shapely.GeometryCollection)):

        def flatten(lst):
            flat_list = []
            for item in lst:
                if isinstance(item, list):
                    flat_list.extend(flatten(item))
                else:
                    flat_list.append(item)
            return flat_list

        def unpack_geom(geometry):
            if not hasattr(geometry, "geoms"):
                return [geometry]
            return [unpack_geom(gg) for gg in geometry.geoms]

        geometries = flatten(unpack_geom(polygon))
        polygons = list(filter(lambda x: isinstance(x, shapely.Polygon), geometries))
        if len(polygons) == 0:
            return None
        areas = [pp.area for pp in polygons]
        idx = np.argmax(areas)
        return polygons[idx]
    else:
        return None


def patch_to_shapely_polygon(patch: mpatches.Patch):
    polygon = shapely.Polygon(patch.get_verts())
    return to_valid_polygon(polygon)


def load_vessel_geojson(json_path):
    with open(json_path) as file:
        loaded_json = json.load(file)
    shapes = [
        shapely.from_geojson(json.dumps(ss["geometry"]))
        for ss in loaded_json["features"]
    ]
    polygons = [to_valid_polygon(ss) for ss in shapes]
    return polygons


def filter_label_area(label_img, area_min, area_max):
    if np.all(label_img == 0):
        return label_img
    ids, counts = da.compute(
        *da.unique(
            da.from_array(label_img, chunks=1000, name=False),
            return_counts=True,
        )
    )
    filtered = np.where((counts >= area_min) & (counts < area_max), ids, 0)
    indexer = np.arange(ids.max() + 1)
    indexer[ids] = filtered
    return indexer[label_img].astype(np.int32)


def find_low_high(img_path, plot=False):
    img = tifffile.imread(img_path, key=[16, 17])

    log_img = np.sum(np.log1p(np.clip(img.astype("float32") - 200, 0, None)), axis=0)
    try:
        low_high = _find_low_high(log_img, plot=plot)
    except Exception as e:
        print()
        print(img_path)
        print(e)
        print()
        raise (e)
    if plot:
        plt.gca().set_title(pathlib.Path(img_path).name)
    return low_high


def _find_low_high(img, plot=False):
    import scipy.signal

    bins = np.linspace(0, 20, 101)
    half_step = bins[:2].mean()
    density, _ = np.histogram(img[::10, ::10].ravel(), bins, density=True)
    xx = bins[:100] + half_step

    density_smooth = skimage.filters.gaussian(density.reshape(1, -1), 1)
    vallies, _ = scipy.signal.find_peaks(-1.0 * density_smooth.flatten())
    peaks, _ = scipy.signal.find_peaks(density_smooth.flatten())
    if plot:
        _, ax = plt.subplots()
        ax.plot(xx, density)
        ax.plot(xx[vallies], density[vallies], "x")
        ax.plot(xx[peaks], density[peaks], "o")
        ax.set_xlim(5, 20)

    try:
        low = np.min(xx[vallies][xx[vallies] > 8.5])
    except ValueError:
        low = np.max(xx[vallies][xx[vallies] <= 8.5])
    high = np.min(xx[peaks][xx[peaks] > low])
    return low, high


def segment_tubule(img_path, roi_path, vessel_json_path, threshold, ring_size_pixel=30):
    img_path = pathlib.Path(img_path)

    img = tifffile.imread(img_path, key=[16, 17])
    img_tissue = np.sum(np.log1p(np.clip(img.astype("float32") - 200, 0, None)), axis=0)
    img = None

    _, mask = cv2.connectedComponents(
        (cv2.GaussianBlur(img_tissue, (0, 0), 4) < threshold).astype("uint8"),
        connectivity=8,
    )

    AREA_MIN = (3 / 0.325) ** 2 * np.pi
    AREA_MAX = 1_000**2

    mask = filter_label_area(mask, AREA_MIN, AREA_MAX)

    _vessel_polygons = load_vessel_geojson(vessel_json_path)
    vessel_clearance = (
        shapely_polygons_to_mask(
            [to_valid_polygon(pp.buffer(20)) for pp in _vessel_polygons], mask.shape
        )
        > 0
    )
    vessel_mask = shapely_polygons_to_mask(_vessel_polygons, mask.shape).astype("int32")
    mask[vessel_clearance] = 0
    mask[vessel_mask > 0] = vessel_mask[vessel_mask > 0] + mask.max()

    gl_center, gs_center = [
        mm.astype("int32")
        for mm in glomerulus_roi_path_to_mask(
            roi_path, buffer_size=-20, mask_shape=mask.shape
        )
    ]
    g_expand = roi_path_to_mask(roi_path, buffer_size=20, mask_shape=mask.shape) > 0

    mask[g_expand] = 0
    max_id = mask.max()
    gl_center[gl_center > 0] += max_id
    gs_center[gs_center > 0] += max_id

    mask += gl_center
    mask += gs_center
    mask_filtered = filter_label_area(mask, AREA_MIN, AREA_MAX)

    # record object type
    # {1: tubule, 2: blood vessel, 3: glomerulus, 4: sclerosed glomerulus}
    type_mask = np.zeros(mask_filtered.shape, dtype="uint8")
    is_obj = mask_filtered > 0
    type_mask[is_obj] = 1
    type_mask[is_obj & (vessel_mask > 0)] = 2
    type_mask[is_obj & (gl_center > 0)] = 3
    type_mask[is_obj & (gs_center > 0)] = 4

    write_kwargs = dict(
        pixel_size=0.325,
        downscale_factor=2,
        is_mask=True,
        compression="zlib",
        save_RAM=True,
    )
    # -------------------------------- tubule mask ------------------------------- #
    out_dir = pathlib.Path(
        r"Z:\yc00000\computation\YC-20241105-kidney-segmentation\tubule-mask"
    )
    palom.pyramid.write_pyramid(
        [da.from_array(mask_filtered.astype("int32"), chunks=1024, name=False)],
        out_dir / f"{img_path.name.split('-')[0]}-tubule_mask.ome.tif",
        **write_kwargs,
    )
    # ----------------------------- object type mask ----------------------------- #
    out_dir_type = pathlib.Path(
        r"Z:\yc00000\computation\YC-20241105-kidney-segmentation\tubule-type-mask"
    )
    palom.pyramid.write_pyramid(
        [da.from_array(type_mask, chunks=1024, name=False)],
        out_dir_type / f"{img_path.name.split('-')[0]}-tubule_type_mask.ome.tif",
        **write_kwargs,
    )
    # ------------------------ tubule ring mask (dilated) ------------------------ #
    out_dir_ring = pathlib.Path(
        r"Z:\yc00000\computation\YC-20241105-kidney-segmentation\tubule-ring-mask"
    )
    mask_derived = (
        skimage.segmentation.expand_labels(mask_filtered, ring_size_pixel)
        - mask_filtered
    )
    palom.pyramid.write_pyramid(
        [da.from_array(mask_derived.astype("int32"), chunks=1024, name=False)],
        out_dir_ring
        / f"{img_path.name.split('-')[0]}-tubule_ring_mask_{ring_size_pixel}.ome.tif",
        **write_kwargs,
    )
    # ---------------------- tubule epithelial mask (eroded) --------------------- #
    out_dir_epi = pathlib.Path(
        r"Z:\yc00000\computation\YC-20241105-kidney-segmentation\tubule-epithelial-mask"
    )
    footprint = skimage.morphology.disk(0.5 * ring_size_pixel)
    mask_derived = skimage.morphology.erosion(mask_filtered, footprint=footprint)
    mask_filtered -= mask_derived
    palom.pyramid.write_pyramid(
        [da.from_array(mask_filtered.astype("int32"), chunks=1024, name=False)],
        out_dir_epi
        / f"{img_path.name.split('-')[0]}-tubule_epithelial_mask_{ring_size_pixel}.ome.tif",
        **write_kwargs,
    )

    return out_dir / f"{img_path.name.split('-')[0]}-tubule_mask.ome.tif"


filepaths = (
    open(
        r"Z:\yc00000\computation\YC-20241105-kidney-segmentation\script-processing\files-ashlar.csv"
    )
    .read()
    .replace('"', "")
    .strip()
    .split("\n")
)
filepaths = {pathlib.Path(ff).name.split("-")[0]: ff for ff in filepaths}

rois = {
    ii: next(
        pathlib.Path(
            r"X:\cycif-production\152-Kidney_Imaging_ProspectiveCasesRound2-2\ROI export"
        ).glob(f"{ii}*-rois.csv")
    )
    for ii in filepaths.keys()
}

geojsons = {
    ii: next(
        pathlib.Path(
            r"X:\cycif-production\152-Kidney_Imaging_ProspectiveCasesRound2-2\ROI-vessel"
        ).glob(f"{ii}*Image0.geojson")
    )
    for ii in filepaths.keys()
}


slides = list(filepaths.keys())
subset = slides[:]


def run_segment_tubule():
    def parallel_wrap(img_path, roi_path, json_path):
        low, high = find_low_high(img_path, plot=False)

        threshold = low * 0.99
        return segment_tubule(img_path, roi_path, json_path, threshold)

    joblib.Parallel(n_jobs=8, verbose=1)(
        joblib.delayed(parallel_wrap)(filepaths[nn], rois[nn], geojsons[nn])
        for nn in subset
    )


def viz():
    import napari

    filepaths = (
        open(
            r"Z:\yc00000\computation\YC-20241105-kidney-segmentation\script-processing\files-ashlar.csv"
        )
        .read()
        .replace('"', "")
        .strip()
        .split("\n")
    )
    filepaths = {pathlib.Path(ff).name.split("-")[0]: ff for ff in filepaths}

    slides = list(filepaths.keys())
    subset = slides[::15]

    out_mask_dir = pathlib.Path(
        r"Z:\yc00000\computation\YC-20241105-kidney-segmentation\tubule-mask"
    )
    masks = {ii: next(out_mask_dir.glob(f"{ii}-tubule_mask.ome.tif")) for ii in subset}
    # out_ring_mask_dir = pathlib.Path(
    #     r"Z:\yc00000\computation\YC-20241105-kidney-segmentation\tubule-ring-mask"
    # )
    # ring_masks = {
    #     ii: next(out_ring_mask_dir.glob(f"{ii}-tubule_ring_mask_30.ome.tif"))
    #     for ii in subset
    # }

    v = napari.Viewer()
    mask_readers = [palom.reader.OmePyramidReader(masks[nn]) for nn in subset]
    # ring_mask_readers = [palom.reader.OmePyramidReader(ring_masks[nn]) for nn in subset]
    img_readers = [palom.reader.OmePyramidReader(filepaths[nn]) for nn in subset]

    # for mm, rr, mr in zip(mask_readers, img_readers, ring_mask_readers):
    for mm, rr in zip(mask_readers, img_readers):
        v.add_image(
            [pp[[16, 17]] for pp in rr.pyramid],
            name=rr.path.name.split("-")[0],
            visible=False,
            contrast_limits=(100, 6000),
            channel_axis=0,
        )
        v.add_labels(mm.pyramid, visible=False, name="mask")
        # v.add_labels(mr.pyramid, visible=False, name="ring mask")


def dev():
    g_center, mask, AREA_MIN, AREA_MAX = None
    contours, _ = cv2.findContours(
        g_center.astype("int32"), cv2.RETR_FLOODFILL, cv2.CHAIN_APPROX_NONE
    )
    g_contour = cv2.drawContours(
        np.zeros_like(g_center), contours, contourIdx=-1, color=1, thickness=1
    ).astype("bool")

    mask |= (g_center > 0).astype("bool")
    mask[g_contour] = False

    _, mask = cv2.connectedComponents(mask.astype("uint8"), connectivity=8)

    mask_filtered = filter_label_area(mask, AREA_MIN, AREA_MAX)
