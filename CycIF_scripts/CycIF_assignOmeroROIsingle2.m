function newtable = CycIF_assignOmeroROIsingle2(data1,filename,scale1,nameflag,marker1,flag2)
%% Script to read & assign ROI from Omero ROI (Yu-AN's script)
%  apply single ROI csv & data table
%  default scale1 = 0.65
%  flag2 = false (non-verbose)
%  Jerry Lin 2023/10/03
%  Updated 2023/12/08

if nameflag 
    data1.ROIname = repmat({'none'},size(data1,1),1);
end

data1{:,marker1} = zeros(size(data1,1),1);

p = 1;
[points,name1] = CycIF_omeROI2pointsText(filename,p,false);

if nameflag 
    if isnumeric(name1)
        data1.ROIname = zeros(size(data1,1),1);
    else
        data1.ROIname = repmat({'none'},size(data1,1),1);
    end
end

if flag2  %Verbose
    tic;
end

while ~isempty(points)
    if flag2  %Verbose
        disp(size(points,1));
    end
    points = points * scale1;
    
    %----simplify points------
    if(size(points,1)>100)
        flag1 = false(size(points,1),1);
        flag1(1:3:end) = true;
        points = points(flag1,:);
    end
    
    %---Test and assign points------    
    flag1 = inpolygon(data1.Xt,data1.Yt,points(:,1),points(:,2));
    data1{flag1,marker1} = repmat(p,sum(flag1),1);
    if nameflag==1
        data1.ROIname(flag1) = name1;
    end
    
    %---Read next ROI-------
    p = p+1;
    [points,name1] = CycIF_omeROI2pointsText(filename,p,false);
    if flag2  %Verbose
        toc;
    end
end

newtable = data1;

return
