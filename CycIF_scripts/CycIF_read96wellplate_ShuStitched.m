%% Import CycIF data from text files for Shu's stitched images
% 2019/02/26  Jerry Lin
% 
% Script for importing data from the following text file:
%
%   Results-[Row][Col].txt    Nuclear data
%   
...
mypath = uigetdir('D:\Shu_Caitlin2\Jerry_ShuCaitlinPilotExp2_Plate3_20210315\Jerry_ShuCaitlinPilotExp2_Plate3_20210315_1\reg');
...
for r=start_r:end_r
    for c=start_c:end_c
        ...
        filename = strjoin(strcat(mypath,'\Result-',well,'.csv'));
...
% end
temp2 = cellfun(@(X)strsplit(X,':'),temp.Label,'UniformOutput',false);
temp2 = vertcat(temp2{:});
temp.Channel = temp2(:,3);
%temp.Channel = cellfun(@(x) x(length(x)-4:length(x)),temp.Label,'UniformOutput',false);
...
welldata{r,c} = temp;
        ...
        DAPI = welldata{r,c}.Mean(strcmp(welldata{r,c}.Channel,'DAPI'));
        AREA = allchs(:,i+1);
        channels(i+2) = {'IntDAPI'};
        allchs(:,i+2) = DAPI .* AREA;

        channels(i+3) = {'X'};
        allchs(:,i+3) = welldata{r,c}.X(strcmp(welldata{r,c}.Channel,channels{1}));
        channels(i+4) = {'Y'};
        allchs(:,i+4) = welldata{r,c}.Y(strcmp(welldata{r,c}.Channel,channels{1}));
        channels(i+5) = {'Field'};
        allchs(:,i+5) = welldata{r,c}.Field(strcmp(welldata{r,c}.Channel,channels{1}));

        wellsum{r,c}=allchs;
        %tableTemp = array2table(allchs,'VariableNames',channels);
        clearvars allchs DAPI AREA temp1 sizes well temp site temp2;
        end
     end
end
